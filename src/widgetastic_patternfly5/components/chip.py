...
STANDALONE_GROUP_LABEL = "./preceding-sibling::*[contains(@class, '-c-chip-group__label')]"
OLD_CHIP_ROOT = (
    ".//*[(self::div or self::li) and contains(@class, '-c-chip') "
...
class OldOverflowChip(OverflowChip):
    ROOT = (
        ...
        "contains(@class, '-c-chip') and contains(@class, 'pf-m-overflow')]"
    )
...
class ChipGroupToolbar(View):
    ...
    overflow = OldOverflowChip(
        locator=("./div[contains(@class, '-c-chip') and contains(@class, 'pf-m-overflow')]")
    )
